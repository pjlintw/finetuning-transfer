    ...
    for (ds, frac_or_samples), ds_config in zip(dataset_mixer.items(), configs):
        ...
        for split in splits:
            ...
            if ds.endswith(".json") or ds.endswith(".jsonl"):
                ...
                try:
                    if "CohereForAI/aya_dataset/ja" in ds:
                        dataset = load_dataset('CohereForAI/aya_dataset', 'default', split='train').remove_columns("language_code").remove_columns("annotation_type").remove_columns("user_id")
                        dataset = dataset.filter(
                            lambda example: example["language"] == "Japanese"
                        ).remove_columns("language")
                        # dataset = dataset.rename_column("inputs", "inputs").rename_column("targets", "targets")
                        print(dataset.column_names)
                        print(len(dataset))
                    
                    elif "CohereForAI/aya_dataset/si" in ds:
                        dataset = load_dataset('CohereForAI/aya_dataset', 'default', split='train').remove_columns("language_code").remove_columns("annotation_type").remove_columns("user_id")
                        dataset = dataset.filter(
                            lambda example: example["language"] == "Sinhala"
                        ).remove_columns("language")
                        # dataset = dataset.rename_column("inputs", "inputs").rename_column("targets", "targets")
                        print(dataset.column_names)
                        print(len(dataset))

                    elif "CohereForAI/aya_dataset/plt" in ds:
                        dataset = load_dataset('CohereForAI/aya_dataset', 'default', split='train').remove_columns("language_code").remove_columns("annotation_type").remove_columns("user_id")
                        dataset = dataset.filter(
                            lambda example: example["language"] == "Plateau Malagasy"
                        ).remove_columns("language")
                        # dataset = dataset.rename_column("inputs", "inputs").rename_column("targets", "targets")
                        print(dataset.column_names)
                        print(len(dataset))

                    elif "openai/gsm8k" in ds:
                        dataset = load_dataset(ds, 'main', split='train')
                        dataset = dataset.rename_column("question", "inputs").rename_column("answer", "targets")
                        print(dataset.column_names)
                        print(len(dataset))

                    elif "kunishou/HelpSteer2-20k-ja" in ds:
                        dataset = load_dataset(ds, split='train').remove_columns("index").remove_columns("prompt").remove_columns("response").remove_columns("correctness").remove_columns("helpfulness").remove_columns("verbosity").remove_columns("complexity").remove_columns("coherence")
                        dataset = dataset.rename_column("prompt_ja", "inputs").rename_column("response_ja", "targets")
                        print(dataset.column_names)
                        print(len(dataset))
